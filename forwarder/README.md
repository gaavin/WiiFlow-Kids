...
The glass wordmark, sky gradient and taglines are generated by
`scripts/kids_channel.py` at the exact TPL sizes the `.brlyt` panes expect.
`tools/rebuild_channel.py` patches those TPLs into the existing WAD and
fake-signs it. Layout, animation and sound are never rewritten.
